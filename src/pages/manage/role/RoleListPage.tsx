...
import { useDeleteRoleById, useGetAllRole } from "@/gen/endpoints/role/role";
import type { RoleResponseDTO } from "@/gen/models";
...
export const RoleListPage = () => {
  const keys: (keyof RoleResponseDTO)[] = [
    "id",
    "name",
    "createdAt",
    "updatedAt",
  ...
  const schemaColumns = createColumnsFromType<RoleResponseDTO>(keys, [
    {
      key: "createdAt",
  ...
  const columns = [
    createSelectionColumn<RoleResponseDTO>(),
    ...schemaColumns,
    createActionColumn<RoleResponseDTO>({
      onEdit: (row) => navigate({ to: `/manage/role/update/${row.id}` }),
    ...
    <>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Button
          ...
          onClick={() => navigate({ to: "/manage/role/create" })}
        >
          ...
          <span className="relative font-semibold">Create role</span>
        </Button>
      </div>
      ...
      <DataTable name="Role Management" table={table} density={density} />
      <PaginationBar
        className="mt-2"
